...
namespace Yahtzee
{
    ...
    {
        public static int Calculate(Roll roll, Combination combination)
        {
            if (roll == null)
            {
        ...
        private static bool IsFullHouse(int[] dicesValues)
        {
            return HasGivenOccurrences(dicesValues,5) 
                   || (dicesValues.GroupBy(x => x).Any(x => x.Count() == 3) && dicesValues.Distinct().Count() == 2);
        }

        private static bool HasGivenOccurrences(int[] dicesValues, int numberOfOccurrences)
        {
            return dicesValues.GroupBy(x => x).Any(x => x.Count() >= numberOfOccurrences);
        }

        private static bool IsSmallStraight(int[] dicesValues)
        {
